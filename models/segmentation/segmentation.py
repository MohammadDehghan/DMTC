...
from .. import resnet
from .deeplab import DeepLabV3Head, DeepLabV2Head, DeepLab, ReconHead, representation
...
def _segm_resnet(name, backbone_name, num_classes, aux, recon_loss, pretrained_backbone=True):
    ...
    model_map = {
        ...
        'deeplabv2_contrastive': (DeepLabV2Head, DeepLab, representation),
        'fcn': (FCNHead, FCN),
    }
    ...
    if name == 'deeplabv2_contrastive':
        rep = model_map[name][2](inplanes, num_classes)
    else:
        rep = None

    model = base_model(backbone, classifier, rep, aux_classifier, recon_classifier)
    return model
...
def deeplabv2_contrastive_resnet101(pretrained=False, progress=True,
                        num_classes=21, aux_loss=None, recon_loss=False, **kwargs):
    """Constructs a DeepLabV3 model with a ResNet-101 backbone.
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _load_model('deeplabv2_contrastive', 'resnet101', pretrained, progress, num_classes, aux_loss,
                       recon_loss, **kwargs)
...
def deeplab_v2_contrastive(num_classes):
    # Define deeplabV2 with ResNet101(With only ImageNet pretraining)
    return deeplabv2_contrastive_resnet101(pretrained=False, num_classes=num_classes, recon_loss=False)
